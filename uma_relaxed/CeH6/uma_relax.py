...
print(f"Starting UMA relaxation for CeH6")
...
opt = BFGS(atoms, trajectory='CeH6_relaxation.traj')
opt.run(fmax=0.05, steps=200)

# Save relaxed structure
write('CeH6_relaxed.cif', atoms, format='cif')
write('CeH6_relaxed_POSCAR', atoms, format='vasp')
...
print(f"Relaxation completed for CeH6")
print(f"Final energy: {final_energy:.6f} eV")
print(f"Final volume: {final_volume:.3f} Å³")
print(f"Final stress: {final_stress}")

# Save results
with open('CeH6_relaxation_results.txt', 'w') as f:
    f.write(f"Compound: CeH6
")
...
print(f"Results saved to CeH6_relaxation_results.txt")
