...
print(f"Starting UMA relaxation for CaH6")
...
opt = BFGS(atoms, trajectory='CaH6_relaxation.traj')
opt.run(fmax=0.05, steps=200)

# Save relaxed structure
write('CaH6_relaxed.cif', atoms, format='cif')
write('CaH6_relaxed_POSCAR', atoms, format='vasp')
...
print(f"Relaxation completed for CaH6")
print(f"Final energy: {final_energy:.6f} eV")
print(f"Final volume: {final_volume:.3f} Å³")
print(f"Final stress: {final_stress}")

# Save results
with open('CaH6_relaxation_results.txt', 'w') as f:
    f.write(f"Compound: CaH6
")
...
print(f"Results saved to CaH6_relaxation_results.txt")
