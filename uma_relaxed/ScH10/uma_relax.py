...
print(f"Starting UMA relaxation for ScH10")
...
opt = BFGS(atoms, trajectory='ScH10_relaxation.traj')
opt.run(fmax=0.05, steps=200)

# Save relaxed structure
write('ScH10_relaxed.cif', atoms, format='cif')
write('ScH10_relaxed_POSCAR', atoms, format='vasp')
...
print(f"Relaxation completed for ScH10")
print(f"Final energy: {final_energy:.6f} eV")
print(f"Final volume: {final_volume:.3f} Å³")
print(f"Final stress: {final_stress}")

# Save results
with open('ScH10_relaxation_results.txt', 'w') as f:
    f.write(f"Compound: ScH10
")
...
print(f"Results saved to ScH10_relaxation_results.txt")
