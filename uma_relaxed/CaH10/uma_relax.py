...
print(f"Starting UMA relaxation for CaH10")
...
opt = BFGS(atoms, trajectory='CaH10_relaxation.traj')
opt.run(fmax=0.05, steps=200)

# Save relaxed structure
write('CaH10_relaxed.cif', atoms, format='cif')
write('CaH10_relaxed_POSCAR', atoms, format='vasp')
...
print(f"Relaxation completed for CaH10")
print(f"Final energy: {final_energy:.6f} eV")
print(f"Final volume: {final_volume:.3f} Å³")
print(f"Final stress: {final_stress}")

# Save results
with open('CaH10_relaxation_results.txt', 'w') as f:
    f.write(f"Compound: CaH10
")
...
print(f"Results saved to CaH10_relaxation_results.txt")
